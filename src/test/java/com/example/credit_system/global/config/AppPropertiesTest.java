...
class AppPropertiesTest {
    ...
    void 유효한_heartbeat_설정은_생성된다() {
        assertThatCode(() -> new AppProperties.Heartbeat(10, 5)).doesNotThrowAnyException();
    }

    @Test
    void refresh_interval이_timeout_이상이면_거부한다() {
        assertThatThrownBy(() -> new AppProperties.Heartbeat(10, 15))
    ...
    void refresh_interval이_timeout과_같아도_거부한다() {
        assertThatThrownBy(() -> new AppProperties.Heartbeat(10, 10))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void timeout이_1_미만이면_거부한다() {
        assertThatThrownBy(() -> new AppProperties.Heartbeat(0, 5))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void refresh_interval이_1_미만이면_거부한다() {
        assertThatThrownBy(() -> new AppProperties.Heartbeat(10, 0))
                .isInstanceOf(IllegalArgumentException.class)
